...
public class Report_DashboardComposite extends Composite implements IForm, ClickHandler, ChangeHandler, SelectionHandler<Integer>
{
	...
	private Label						topLabel					= new Label (FM.getProjectTitle () + " Dashboard");
	...
	private void displaySummary ()
	{
		...
		try
		{
			...
			{
				Widget w = iter.next ();
				String encounter = specifyFormTypeCheckBox.getValue () ? FMClient.get (encounterTypeComboBox) : "";
				if (w instanceof TextBox)
				{
					TextBox textBox = (TextBox) w;
			...
			{
				String option = FMClient.get (patientOptionsComboBox);
				String selection = FMClient.get (patientOptionsListComboBox);
	...
	public void onChange (ChangeEvent event)
	{
		...
		if (sender == patientOptionsComboBox)
		{
			...
			if (FMClient.get (patientOptionsComboBox).equals ("AGE"))
			{
			...
			else if (FMClient.get (patientOptionsComboBox).equals ("GENDER"))
			{
