...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "fieldmonitoring" because\n';
  ...
  $wnd.__gwt_activeModules['fieldmonitoring'] = {
    'moduleName' : 'fieldmonitoring',
    'bindings' : function() {
      return {};
    }
  };
 ...
 * "/fieldmonitoring.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['fieldmonitoring'].superdevmode = true;

  var expectedSuffix = '/fieldmonitoring.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + fieldmonitoring + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:fieldmonitoring';
  var devModeSessionKey = '__gwtDevModeSession:fieldmonitoring';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/fieldmonitoring/fieldmonitoring.nocache.js';
  ...
  $wnd.__gwt_activeModules['fieldmonitoring'].canRedirect = true;
  $wnd.__gwt_activeModules['fieldmonitoring'].superdevmode = true;
  ...
  css = (moduleIdx == 1 ? css : '') +
    ".gwt-DevModeModule-" + moduleIdx + ".gwt-DevModeCompiling div:before{" +
      "content:'COMPILING fieldmonitoring';" +
      "font-size:24px;" +
      "color:#d2d9ee;" +
    "}";
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/fieldmonitoring?user.agent=' + ua + '&_callback=' + callback;
