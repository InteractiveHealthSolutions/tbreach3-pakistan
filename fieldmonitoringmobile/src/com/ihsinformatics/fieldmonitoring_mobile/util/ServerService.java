...
package com.ihsinformatics.fieldmonitoring_mobile.util;
...
import com.ihsinformatics.fieldmonitoring_mobile.App;
...
public class ServerService {
	...
	public OpenMrsObject getLocation(String name) {
		...
		if (id == null) {
			try {
				...
				// if(response.contains("Application Offline"))
				// {
				//
				// }
	...
	public String updateLocations() {
		...
		try {
			JSONArray retrievedLocations = getAllLocations();
			if (retrievedLocations != null) {
				// delete location Metadata
				isDeleted = dbUtil.deleteTypeIdentifiersTable(
						Metadata.METADATA_TABLE, Metadata.LOCATION);

				if (isDeleted) {
					for (int i = 0; i < retrievedLocations.length(); i++) {
						JSONObject locationObj = new JSONObject(
								retrievedLocations.get(i).toString());
						ContentValues values = new ContentValues();
						values.put("id", locationObj.getInt("id"));
						values.put("type", Metadata.LOCATION);
						values.put("name", locationObj.getString("name"));
						Log.i(TAG, "id = " + locationObj.getInt("id")
								+ ", name = " + locationObj.getString("name"));
						dbUtil.insert(Metadata.METADATA_TABLE, values);
					}
				}
	...
	public JSONArray getAllLocations() {
		...
		try {
			json.put("app_ver", App.getVersion());
			json.put("form_name", FormType.GET_ALL_LOCATION);
			// long count = dbUtil.getTableSize("location");
			String response = get("?content=" + JsonUtil.getEncodedJson(json));

			JSONObject jsonResponse = JsonUtil.getJSONObject(response);
			locations = new JSONArray(jsonResponse.get("locations").toString());
		} catch (JSONException e) {
			Log.e(TAG, e.getMessage());
		} catch (UnsupportedEncodingException e) {
			Log.e(TAG, e.getMessage());
	...
	public String saveVisits(String encounterType, ContentValues values,
			String[][] observations) {
		String response = "";
		String location = values.getAsString("location");
		String formDate = values.getAsString("formDate");
		String formName = values.getAsString("formName");

		String locationId = "";

		// if (encounterType.equals(FormType.FIRST_VISIT))
		// {
		locationId = values.getAsString("locationId");
		// }
