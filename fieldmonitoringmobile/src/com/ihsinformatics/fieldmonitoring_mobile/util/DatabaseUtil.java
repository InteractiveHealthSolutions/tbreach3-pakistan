...
public class DatabaseUtil extends SQLiteOpenHelper {
	...
	public synchronized boolean deleteTypeIdentifiersTable(
			String metadataTable, String type) {
		boolean check = false;
		DatabaseUtil util = new DatabaseUtil(context);
		SQLiteDatabase db = getWritableDatabase();
		db.execSQL("delete from " + metadataTable + " where type = '" + type
				+ "';");
		db.close();
		check = true;
		Log.i(TAG, "Records deleted from table: " + metadataTable + "for type "
				+ type);
		// SQLiteDatabase writableDatabase = util.getWritableDatabase ();
		// long result = writableDatabase.delete (table, whereClause,
		// whereArgs);
		// writableDatabase.close ();
		// boolean check = (result != -1);
		// if (check)
	...
	public long getTableSize(String tableName) {
		...
		SQLiteStatement s = writableDatabase
				.compileStatement("select count(*) from ='" + tableName + "'; ");
