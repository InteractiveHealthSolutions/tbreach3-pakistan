...
set @max := (select max(obs_id) from obs where year(date_created) = 2014 and month(date_created) = 1);
delete from obs where obs_id < @max;
set @max := (select max(obs_id) from obs where year(date_created) = 2014 and month(date_created) = 2);
delete from obs where obs_id < @max;
set @max := (select max(obs_id) from obs where year(date_created) = 2014 and month(date_created) = 3);
delete from obs where obs_id < @max;
set @max := (select max(obs_id) from obs where year(date_created) = 2014 and month(date_created) = 4);
delete from obs where obs_id < @max;
set @max := (select max(obs_id) from obs where year(date_created) = 2014 and month(date_created) = 5);
delete from obs where obs_id < @max;
set @max := (select max(obs_id) from obs where year(date_created) = 2014 and month(date_created) = 6);
delete from obs where obs_id < @max;
set @max := (select max(obs_id) from obs where year(date_created) = 2014 and month(date_created) = 7);
delete from obs where obs_id < @max;
set @max := (select max(obs_id) from obs where year(date_created) = 2014 and month(date_created) = 8);
delete from obs where obs_id < @max;
set @max := (select max(obs_id) from obs where year(date_created) = 2014 and month(date_created) = 9);
delete from obs where obs_id < @max;
set @max := (select max(obs_id) from obs where year(date_created) = 2014 and month(date_created) = 10);
delete from obs where obs_id < @max;
set @max := (select max(obs_id) from obs where year(date_created) = 2014 and month(date_created) = 11);
delete from obs where obs_id < @max;
set @max := (select max(obs_id) from obs where year(date_created) = 2014 and month(date_created) = 12);
delete from obs where obs_id < @max;
...
set @count = 4000000;
set @count_obs = 40000000;

Update openmrs.active_list_type SET active_list_type.active_list_type_id = (active_list_type.active_list_type_id + @count) 
where (date_created BETWEEN @start_date AND @end_date);

Update openmrs.cohort SET cohort.cohort_id = (cohort.cohort_id + @count) where
(date_created BETWEEN @start_date AND @end_date) OR (date_changed BETWEEN @start_date AND @end_date);

Update openmrs.concept SET concept.concept_id = (concept.concept_id + @count) where
(date_created BETWEEN @start_date AND @end_date) OR (date_changed BETWEEN @start_date AND @end_date);

Update openmrs.concept_answer SET concept_answer.concept_answer_id = (concept_answer.concept_answer_id + @count) where
(date_created BETWEEN @start_date AND @end_date);

Update openmrs.concept_class SET concept_class.concept_class_id = (concept_class.concept_class_id + @count) where
(date_created BETWEEN @start_date AND @end_date);

-- concept_complex

Update openmrs.concept_datatype SET concept_datatype.concept_datatype_id = (concept_datatype.concept_datatype_id + @count) where
(date_created BETWEEN @start_date AND @end_date);

Update openmrs.concept_description SET concept_description.concept_description_id = (concept_description.concept_description_id + @count) where
(date_created BETWEEN @start_date AND @end_date) OR (date_changed BETWEEN @start_date AND @end_date);

Update openmrs.concept_map_type SET concept_map_type.concept_map_type_id = (concept_map_type.concept_map_type_id + @count) where
(date_created BETWEEN @start_date AND @end_date) OR (date_changed BETWEEN @start_date AND @end_date);

Update openmrs.concept_name SET concept_name.concept_name_id = (concept_name.concept_name_id + @count) where
(date_created BETWEEN @start_date AND @end_date);

Update openmrs.concept_name_tag SET concept_name_tag.concept_name_tag_id = (concept_name_tag.concept_name_tag_id + @count) where
(date_created BETWEEN @start_date AND @end_date);

-- concept_name_tag_map
-- concept_numeric

Update openmrs.concept_proposal SET concept_proposal.concept_proposal_id = (concept_proposal.concept_proposal_id + @count) where
(date_created BETWEEN @start_date AND @end_date) OR (date_changed BETWEEN @start_date AND @end_date);

Update openmrs.concept_reference_map SET concept_reference_map.concept_map_id = (concept_reference_map.concept_map_id + @count) where
(date_created BETWEEN @start_date AND @end_date) OR (date_changed BETWEEN @start_date AND @end_date);

Update openmrs.concept_reference_source set concept_source_id = (concept_source_id + @count) where
(date_created BETWEEN @start_date AND @end_date);

Update openmrs.concept_reference_term set concept_reference_term_id = (concept_reference_term_id + @count) where
(date_created BETWEEN @start_date AND @end_date) OR (date_changed BETWEEN @start_date AND @end_date);

Update openmrs.concept_reference_term_map set concept_reference_term_map_id = (concept_reference_term_map_id + @count) where
(date_created BETWEEN @start_date AND @end_date) OR (date_changed BETWEEN @start_date AND @end_date);

Update openmrs.concept_set set concept_set_id = (concept_set_id + @count) where
(date_created BETWEEN @start_date AND @end_date);
...
Update openmrs.drug set drug_id = (drug_id + @count) where
(date_created BETWEEN @start_date AND @end_date) OR (date_changed BETWEEN @start_date AND @end_date);

-- drug_ingredient

Update openmrs.orders set order_id = (order_id + @count) where
(date_created BETWEEN @start_date AND @end_date);

-- Update openmrs.drug_order set order_id = (order_id + @count);

Update openmrs.encounter set encounter_id= (encounter_id + @count) where
(date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);

Update openmrs.encounter_provider set encounter_provider_id= (encounter_provider_id + @count) where
(date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);

Update openmrs.encounter_role set encounter_role_id= (encounter_role_id + @count) where
(date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);

Update openmrs.encounter_type set encounter_type_id= (encounter_type_id + @count) where
(date_created BETWEEN start_date AND end_date);

Update openmrs.field set field_id = (field_id + @count) where
(date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);

-- field_answer

Update openmrs.field_type set field_type_id = (field_type_id + @count) where
(date_created BETWEEN start_date AND end_date);

Update openmrs.form set form_id = (form_id + @count) where
(date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);

Update openmrs.form_field set form_field_id = (form_field_id + @count) where
(date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);
...
Update openmrs.location set location_id = (location_id + @count) where
(date_created BETWEEN start_date AND end_date);

Update openmrs.location_attribute set location_attribute_id = (location_attribute_id + @count) where
(date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);

Update openmrs.location_attribute_type set location_attribute_type_id = (location_attribute_type_id + @count) where
(date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);

Update openmrs.location_tag set location_tag_id = (location_tag_id + @count) where
(date_created BETWEEN start_date AND end_date);

-- location_tag_map

Update openmrs.note set note_id = (note_id + @count) where
(date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);

Update openmrs.notification_alert set alert_id = (alert_id + @count) where
(date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);

Update openmrs.notification_template set template_id = (template_id + @count);

Update openmrs.obs set obs_id = (obs_id + @count_obs) where
(date_created BETWEEN start_date AND end_date);

Update openmrs.order_type set order_type_id = (order_type_id + @count) where
(date_created BETWEEN start_date AND end_date);

Update openmrs.person set person_id = (person_id + @count) where
(date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);

-- Update openmrs.patient set patient_id = (patient_id + @count) where
-- (date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);

Update openmrs.patient_identifier set patient_identifier_id = (patient_identifier_id  + @count) where
(date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);

Update openmrs.patient_identifier_type set patient_identifier_type_id = (patient_identifier_type_id + @count) where
(date_created BETWEEN start_date AND end_date);

Update openmrs.patient_program set patient_program_id = (patient_program_id + @count) where
(date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);

Update openmrs.patient_state set patient_state_id = (patient_state_id + @count) where
(date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);

Update openmrs.person_address set person_address_id = (person_address_id + @count) where
(date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);

Update openmrs.person_attribute set person_attribute_id = (person_attribute_id + @count) where
(date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);

Update openmrs.person_attribute_type set person_attribute_type_id = (person_attribute_type_id + @count) where
(date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);

Update openmrs.person_merge_log set person_merge_log_id = (person_merge_log_id + @count) where
(date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);

Update openmrs.person_name set person_name_id = (person_name_id + @count) where
(date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);

-- privilege

Update openmrs.program set program_id = (program_id + @count) where
(date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);

Update openmrs.program_workflow set program_workflow_id = (program_workflow_id + @count) where
(date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);

Update openmrs.program_workflow_state set program_workflow_state_id = (program_workflow_state_id + @count) where
(date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);

Update openmrs.provider set provider_id = (provider_id + @count) where
(date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);

Update openmrs.provider_attribute set provider_attribute_id = (provider_attribute_id + @count) where
(date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);

Update openmrs.provider_attribute_type set provider_attribute_type_id = (provider_attribute_type_id + @count) where
(date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);

Update openmrs.relationship set relationship_id = (relationship_id + @count) where
(date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);

Update openmrs.relationship_type set relationship_type_id = (relationship_type_id + @count) where
(date_created BETWEEN start_date AND end_date);
...
Update openmrs.scheduler_task_config set task_config_id = (task_config_id + @count) where
(date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);

Update openmrs.scheduler_task_config_property set task_config_property_id = (task_config_property_id + @count);

Update openmrs.serialized_object set serialized_object_id = (serialized_object_id + @count) where
(date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);

Update openmrs.users set user_id = (user_id + @count) where
(date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);

Update openmrs.visit set visit_id = (visit_id + @count) where
(date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);

Update openmrs.visit_attribute set visit_attribute_id = (visit_attribute_id + @count) where
(date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);

Update openmrs.visit_attribute_type set visit_attribute_type_id = (visit_attribute_type_id + @count) where
(date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);

Update openmrs.visit_type set visit_type_id = (visit_type_id + @count) where
(date_created BETWEEN start_date AND end_date) OR (date_changed BETWEEN start_date AND end_date);

-- I HAVE NO CLUE WHAT TO DO NEXT. THE reset_increment... PROCEDURE MAKES NO SENSE TO ME

-- ---------------------------------RESTORE OLD OBS DATA----------------------------------------
insert into obs select * from obs_pre_2015;
