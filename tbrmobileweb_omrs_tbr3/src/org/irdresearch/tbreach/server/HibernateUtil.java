...
import org.irdresearch.tbreach.shared.model.LogDataChange;
import org.irdresearch.tbreach.shared.model.LogDataDelete;
import org.irdresearch.tbreach.shared.model.LogDataInsert;
import org.irdresearch.tbreach.shared.model.LogUserLogin;
...
public class HibernateUtil implements Serializable
{
	...
	public HibernateUtil()
	{
		...
		factory = new Configuration().configure("tbreach.cfg.xml").buildSessionFactory();
	...
	public void recordLog(LogType logType, Object obj)
	{
		...
		{
			case DELETE:
				logObj = new LogDataDelete(TBR.getCurrentUser(), new Date(), className, (objClass).cast(obj).toString());
				break;
			case ERROR:
				break;
			case INSERT:
				logObj = new LogDataInsert(TBR.getCurrentUser(), new Date(), className, (objClass).cast(obj).toString());
				break;
			case UPDATE:
				logObj = new LogDataChange(TBR.getCurrentUser(), new Date(), className, (objClass).cast(obj).toString());
				break;
			case LOGIN:
				logObj = new LogUserLogin(TBR.getCurrentUser(), new Date(), null, "");
				break;
			case LOGOUT:
				logObj = new LogUserLogin(TBR.getCurrentUser(), null, new Date(), "");
				break;
		}
		try
		{
